...
Neben der ``Hat-Beziehung`` gibt es die ``Ist-Beziehung``. Sie drückt aus, dass eine ``Klasse`` eine *spezialisierte Version* einer anderen ist - Ein *Hamster* **ist ein** *Tier*. Hierbei unterscheiden wir primär drei Konstrukte:
*   **Klasse:** Eine ``Klasse`` kann **von genau einer** anderen ``Klasse`` *erben* und übernimmt damit deren ``Mitglieder``. Wir können nun weitere ``Mitglieder`` hinzufügen (spezialisieren), welche nur diese ``Klasse`` kennt. Jedes *Tier*, also auch ein *Hamster* hat eine *Representation*, aber ein *Hamster* kann **zusätzlich** einen *Mundspeicher* fuer Essen haben, die im *Tier* fehlt. Die ``Klasse`` *Hamster* nennen wir dann ``abgeleitete Klasse`` und die ``Klasse`` *Tier* die ``Basisklasse``.
  
>**Anmerkung:** Details siehe Skriptum [L01.8-Polymorphismus.md](L03-Polymorphismus.md).

>**Wir merken uns zur Vererbung:**
>1) Die ``Ist-Beziehung`` beschreibt, dass eine ``Klasse`` eine spezialisiere Ausprägung einer anderen ist. Dies wird über ``Vererbung`` realisiert.
>1) Die ``Basisklasse`` beschreibt die *allgemeine* ``Klasse`` und die ``abgeleitete Klasse`` eine *spezialisierte* Ausprägung der ``Basisklasse``.
>3) Der ``abgeleitete Klasse`` werden alle ``Mitglieder`` der ``Basisklasse`` vererbt. 
>4) Die ``Basisklasse`` kennt die ``Mitglieder`` der ``abgeleiteten Klasse`` nicht.