...
[``Operator``](04_Glossar.md#operator), [``Variable``](04_Glossar.md#variable), [``Wert``](04_Glossar.md#wert), [``Typ``](04_Glossar.md#typ), [``String``](04_Glossar.md#string), [``Zuweisungsoperator``](04_Glossar.md#zuweisungsoperator), [``Arithmetische Operatoren``](04_Glossar.md#arithmetische-operatoren), [``Logische Operatoren``](04_Glossar.md#logische-operatoren), [``Bitweise Operatoren``](04_Glossar.md#bitweise-operatoren), [``Vergleichsoperatoren``](04_Glossar.md#vergleichsoperatoren), [``Typecasting``](04_Glossar.md#typecasting), [``Unärer Operator``](04_Glossar.md#unärer-operator), [``Binärer Operator``](04_Glossar.md#binärer-operator), [``Tertiärer Operator``](04_Glossar.md#tertiärer-operator), [``Inkrement``](04_Glossar.md#inkrement), [``Dekrement``](04_Glossar.md#dekrement), [``Verzweigung``](04_Glossar.md#verzweigung), [``Methode``](04_Glossar.md#methode), [``Klasse``](04_Glossar.md#klasse), [``Objekt``](04_Glossar.md#objekt).
...
> **Wir merken uns:**
     ...
     2) Die Verarbeitung findet strikt von rechts nach links statt.
...
> **Wir merken uns:** 
     1) Bei der Division von zwei Ganzzahlen (`int`) schneidet C# die Kommastellen einfach ab (z. B. *5 / 2* ergibt *2*).
     2) Der Modulo-``Operator`` *%* gibt den **Rest einer Division** zurück und kann verwendet werden ob eine Zahl gerade ist (*zahl % 2 == 0*).
...
> **Wir merken uns:** 
    1) Das logische UND (`&&`) ergibt nur dann `true`, wenn **alle** verknüpften Bedingungen wahr sind. 
    2). Das logische ODER (`||`) ergibt bereits `true`, wenn **mindestens eine** der Bedingungen wahr ist. 
    3) Das logische NICHT (`!`) dreht den Wahrheitswert einfach um (aus `true` wird `false` und umgekehrt).
    4) Ein logisches UND kann durch folgenden Zusammenhang in ein logisches ODER verwandelt werden - *a && b* hat die gleichen Wahrheitswerte wie *!(a || b)*.
...
> **Wir merken uns:** 
     1) Wollen wir zwei Werte auf Gleichheit überprüfen, müssen wir zwingend das **doppelte Gleichheitszeichen** `==` verwenden.
     2) Das Ergebnis eines Vergleichs ist immer ein Wahrheitswert - ein *bool* welcher *true* oder *false* sein kann.
...
> **Wir merken uns:** 
    1) `i++` ist lediglich eine praktische Kurzschreibweise für `i = i + 1`. 
    2) Kombinierte Operatoren wie `x += 5` helfen uns, Code kürzer zu fassen (entspricht `x = x + 5`).
...
Warum wir in c# mehr operatoren als in java verwenden wollen.
